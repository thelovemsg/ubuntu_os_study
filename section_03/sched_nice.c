...
static void parent_fn(int nproc){
	...
	for(i=0; i<nproc; i++)
			wait(NULL); // 부모 프로세스는 자식 프로세스가 종료되길 기다리고, 자식 프로세스가 종료되면 그 상태 정보를 회수한다. 
}
...
int main(int argc, char *argv[]){
	...
	int crecord = total / resol;

	struct timespec *logbuf = malloc(crecord * sizeof(struct timespec));
		if(!logbuf)
	...
	for(i = 0, ncreated = 0; i< nproc; i++, ncreated++) {
		...
		if (pids[i] < 0 ){
			...
			if (i == 1)
				nice(5);
			child_fn(i, logbuf, crecord, nloop_per_resol, start);
